 ...
 * @test
 * @bug 1234567
 * @summary BigRamTester
 * @library /test/lib
 * @modules java.base/java.util java.base/java.text java.base/java.util.concurrent
 * @build jdk.test.whitebox.WhiteBox
 * @run driver jdk.test.lib.helpers.ClassFileInstaller jdk.test.whitebox.WhiteBox
 * @run main/othervm -Xbootclasspath/a:. -XX:+UnlockDiagnosticVMOptions -XX:+WhiteBoxAPI
 * -XX:+UseG1GC -Xlog:gc* -Xlog:gc:g1gc.log -Xms4g -Xmx4g  BigRamTester
 */
...
public class BigRamTester {
  ...
  private static final long RESERVED_SPACE = 512L * 1024L * 1024L; // 512 MB

  private static final int MAX_ACCESS_COUNT = 5000000; // 设置最大访问次数
  ...
  private void accessCache(int keys) throws InterruptedException {
    ...
    int access_count = 0;
    while (access_count < MAX_ACCESS_COUNT) {
  ...
  private int createData() {
    // Try and estimate the number of records that we will store to avoid resizing the hashmap many
    // times. This is based on the fixed APPROX_BYTES_PER_RECORD value, which is based on observing
    // the amount of memory used with a fixed value of MAX_LIST_SIZE
    int estimatedNumberOfRecords =
        (int) ((Runtime.getRuntime().freeMemory() - RESERVED_SPACE) / APPROX_BYTES_PER_RECORD)
        + 1000;
    ...
    while (key < Integer.MAX_VALUE) {
      if (Runtime.getRuntime().freeMemory() < RESERVED_SPACE) {
        ...
        System.gc();
        System.out.println(
            "Finished; memory remaining: " + Runtime.getRuntime().freeMemory() + " bytes");
        if (Runtime.getRuntime().freeMemory() < RESERVED_SPACE) {
          break;
        }
      }
  ...
  private void printMemoryUsed(long key, long freeMemoryAtStart) {
    ...
    System.out.println(getDate() + "Created " + key + " keys, using " + (memoryUsed / 1024 / 1024)
        + "MB; that's " + (memoryUsed / key) + " bytes per record; free memory = "
        + (Runtime.getRuntime().freeMemory() / 1024 / 1024) + "MB"
        + "; reserved memory = " + RESERVED_SPACE + " bytes; uptime = " + (uptime / 1000)
        + " seconds");
  }
          ...
          while (true) {
            ...
            synchronized (Statistics.this) {
              ...
              if (hits.get() > 0 && misses.get() > 0) {
                System.out.println(getDate() + "Hit rate = "
                    + ((double) hits.get() / ((double) misses.get() + (double) hits.get()))
                    + "; total hits = " + hits.get() + "; total misses = " + misses.get()
                    + "; total reads = " + (hits.get() + misses.get()) + "; free memory = "
                    + (Runtime.getRuntime().freeMemory() / 1024 / 1024) + "MB"
